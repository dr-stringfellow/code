...
// ----------
//
// Performs selection cuts according to a config file and/or a JSON.
//
...
int main(int argc,char**argv)
{
  ...
  //
  // Skim according to JSON file instead of regular selection cuts?
  //
  // This mode is enabled if there is only a single cut and it contains the term ".json".
  string jsonfile;
  RunRangeManager rrm;
  ...
  while ((dirKey=(TKey*)nextDir())) {
    if (strcmp(dirKey->GetClassName(),"TDirectoryFile") != 0)
      continue;
    TDirectory* idir    = (TDirectory*)dirKey->ReadObj();
    string      dirname = idir->GetName();
    if (dirs.size()>0 && find(dirs.begin(),dirs.end(),dirname) == dirs.end())
      continue;
    ...
    if (jsonfile.empty()) {
      ...
      if(!noeff){
	const double ntotal     = (double)itree->GetEntries();
	const double nselected  = (double)otree->GetEntries();
	const double efficiency = nselected / ntotal;
	stringstream ss;
...
// Reads out the name or expression of a single cut from the vector of cuts.
// "mode" can be "label" or "expression". The selection expression is in a format as understood for example by TTree.
string get_cut(const vector<string>& cuts,unsigned icut,const string& mode)
{
  if (icut>=cuts.size())
    return string("");

  const string tmp(cuts[icut]);

  const size_t pos = tmp.find(':');
  if (pos==string::npos)
    return tmp;

  const string cutlabel = tmp.substr(0,pos);
  const string cutexpression= tmp.substr(pos+1);
