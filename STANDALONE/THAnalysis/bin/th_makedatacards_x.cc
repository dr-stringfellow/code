...
// ---------------------
//
// Create datacards for the limit calculation.
// To understand the usage, look at the contents of ../makedatacards_template/
// An explanation of the datacard format can be found here:
 ...
 //
  ...
  if (dcwrite) {
	...
	if(theta)
      ...
      ss<<setw(ws)<<"process"<<"   "; 
      for (unsigned i=0;i<mcrates.size();i++) {ss<<setw(wp)<<i;} ss<<endl;
      ss<<setw(ws)<<"rate"<<"   "; 
