...
int main(int argc,char**argv)
{
  ...
  for (unsigned int iinput = 0; iinput<inputs.size(); ++iinput) {
	...
	if( JetIsCentral[i]  &&  Matchable(p4hdau1,p4jets[i])<bestdR_hdau1){
	  bestdR_hdau1=Matchable(p4hdau1,p4jets[i]);
	  best_i_pm[0]=i;
	}
	if( JetIsCentral[i]  &&  Matchable(p4hdau2,p4jets[i])<bestdR_hdau2){
	  bestdR_hdau2=Matchable(p4hdau2,p4jets[i]);
	  best_i_pm[1]=i;
	}
	if( JetIsCentral[i]  &&  Matchable(p4btop,p4jets[i])<bestdR_btop){
	  bestdR_btop=Matchable(p4btop,p4jets[i]);
	  best_i_pm[2]=i;
	}
	if( !JetIsTagged[i]  &&  JetIsInReco[i] && Matchable(p4light,p4jets[i])<bestdR_light){
