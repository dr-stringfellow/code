...
//          modified                   24/03/2016 Denise Mueller
///////////////////////////////////////////////////////////////////////////////////////
...
TH1F* my_bin_change(TH1F* h, int nbinsx, double xmin, double xmax, int lowerbin, int higherbin);
...
int main(int argc,char**argv)
{
  ...
  double         xbinmin      = cl.getValue <double>  ("xbinmin",              -1E10);
  double         xbinmax      = cl.getValue <double>  ("xbinmax",              +1E10);
  ...
  // signal scale factor for tHq
  double         signalfac = cl.getValue<double>   ("signalfac",            1.);
  // signal scale factor for tHW
  // dummys
  ...
  bool           setmu     = cl.getValue <bool>    ("setmu",              false);
  ...
  // plotting prefit plots instead of postfit plots
  bool           prefit    = cl.getValue <bool>    ("prefit",            false);
  ...
  bool           changeBinning   = cl.getValue <bool>    ("changeBinning",           false);
  ...
  rootfile = ipath + "/" + rootfile;
   ...
   new TColor(1303,0.53537487589261112, 0.38903115575804437, 0.55080740057370248
	      );
   new TColor(1302,0.75078431373717736, 0.50374086896578463, 0.25235294121153207
              );

   new TColor(1301,0.84599770237417782, 0.6209765700732961, 0.73920801246867462
	      );

   new TColor(1309,0.34509803921568627, 0.49411764705882355, 0.6823529411764706
	      );
  ...
  if(combine)
    { 
      if(prefit)
      {
        idir = (TDirectory*)hfile->Get(("shapes_prefit/" + channel).c_str());
      }
      else
      {
        if(bfit)
	  ...
	  cout << "Checking shapes_fit_s" << endl;
	}
      }
  ...
  while ((key=(TKey*)next())) {
    ...
    cout << "Reading " << tmpchannel << "|" << tmpprocess << "|" <<tmpsys << "|" <<endl;
  ...
  if(theta){
   ...
   //cout << "!!!!!!!!!!!!\n" << channel3 << endl;
   mcsignal.push_back((TH1F*) hfile3->Get((channel3+"__tHq__nominal").c_str()));

  }
  //get signal histos and data histo from mlfit.root
  if(combine){

    
    TFile* hfile2 = new TFile((hpath+histo).c_str(),"READ");
    //    (TLeaf*)hfile2->Get((hpath+"tree_fit_sb/mu").c_str());
   string channel2 = channel;
   //if(channel2.find("ttbar_cr")!=string::npos)
   //  channel2.replace(0,11, "tt");
   //cout << "!!!!!!!!!!!!\n" << channel2 << endl;
   ...
   //cout << "!!!!!!!!!!!!\n" << channel3 << endl;
   //   mcsignal.push_back((TH1F*) hfile3->Get((channel3+"__tHq__nominal").c_str()));
   //mcsignal.push_back((TH1F*) tmpmchist.at(6));

   for (auto const& i : savedhist ){
     cout << "savedhist : " << i << endl; //__tH__nominal 
   }

   /*   TFile* hfile4 = new TFile((hpath+histo).c_str(),"READ");
   string channel4 = channel;
   //if(channel3.find("ttbar_cr")!=string::npos)
   //  channel3.replace(0,11, "tt");
   //cout << "!!!!!!!!!!!!\n" << channel3 << endl;
   mcsignal.push_back((TH1F*) hfile4->Get((channel4+"__tHW__nominal").c_str()));
   */
  }
  ...
  cout << "Found " <<  setw(6) << mcsignal.size() << " signal templates" << endl;
  
  cout << savedsys.size() << endl;
  ...
  for(int i=0; i  < tmpmchist.size(); i++){
    if(combine){
      if(datahist.size()>0) tmpmchist[i] = combine_bin_change(tmpmchist[i], 
  ...
  if(changeBinning){
    //find lower data or MC edge
    Int_t lowestBin_overall = 100;
    Int_t highestBin_overall = -1;
    for (auto const& i : tmpmchist ){
      int lowestBin =  i->FindFirstBinAbove(0.000000001);
      int highestBin =  i->FindLastBinAbove(0.0001);
      if(highestBin > highestBin_overall) highestBin_overall = highestBin;
      if(lowestBin < lowestBin_overall) lowestBin_overall = lowestBin;
    }
    for (auto const& i : datahist ){
      int lowestBin =  i->FindFirstBinAbove(0.0001);
      int highestBin =  i->FindLastBinAbove(0.0001);
      if(highestBin > highestBin_overall) highestBin_overall = highestBin;
      if(lowestBin < lowestBin_overall) lowestBin_overall = lowestBin;
    }
    cout << "Found lowest egde at " << lowestBin_overall << " and highest edge at " << highestBin_overall << endl;
    cout << "Setting up to histo with " << highestBin_overall-lowestBin_overall+1 << " bins from " << datahist[0]->GetXaxis()->GetBinLowEdge(lowestBin_overall) << " to " << datahist[0]->GetXaxis()->GetBinUpEdge(highestBin_overall) << endl;
    
    
    vector<TH1F*> new_tmpmchist;
    vector<TH1F*> new_datahist;
    
    if (xbinmin>-1E10 && xbinmax< 1E10) {
      lowestBin_overall = xbinmin;
      highestBin_overall = xbinmax;
      cout << "Changing binning" << xbinmax << " " << xbinmin << endl;
    }
    
    cout << "lowest: " << lowestBin_overall << " highest: " << highestBin_overall << endl; 
    for (auto  i : tmpmchist ){
      new_tmpmchist.push_back(my_bin_change(i, highestBin_overall-lowestBin_overall+1,
					    datahist[0]->GetXaxis()->GetBinLowEdge(lowestBin_overall),
					    datahist[0]->GetXaxis()->GetBinUpEdge(highestBin_overall),
					    lowestBin_overall,
					    highestBin_overall));
    }
    
    for (auto i : datahist ){
      new_datahist.push_back(my_bin_change(i, highestBin_overall-lowestBin_overall+1,
					   datahist[0]->GetXaxis()->GetBinLowEdge(lowestBin_overall),
					   datahist[0]->GetXaxis()->GetBinUpEdge(highestBin_overall),
					   lowestBin_overall,
					   highestBin_overall));
  ...
  */
  ...
  if (mchist.size()>0) {
    ...
    //Iterate through mc histos and search for signal and plot the postfit signal as line shape
    for (auto const& i : mchist ){ 
      cout << "mchist : " << i->GetName() << endl; //__tH__nominal 
      string tmp_histname = i->GetName();
      if (tmp_histname.find("_tHq_")!=string::npos){
	cout << "Found signal histo " << i->GetName()  << endl;
	mcsignal.push_back((TH1F*) i->Clone());
      }
    }

    for (auto const& i : mchist ){ 
      string tmp_histname = i->GetName();
      if (tmp_histname.find("_tHW_")!=string::npos){
	cout << "Found signal histo " << i->GetName()  << endl;
	mcsignal.push_back((TH1F*) i->Clone());
      }
    }
    ...
    //if (xmin>-1E10 && xmax< 1E10) hmc->GetXaxis()->SetRangeUser(xmin, xmax);
    hmc->Draw("HIST");
    hmc->GetHistogram()->SetYTitle("Events/Bin");
    hmc->SetMinimum(minimum);
    hmc->SetMaximum(maximum*1.3);
    hmc->GetHistogram()->SetMinimum(minimum);
    hmc->GetHistogram()->SetMaximum(maximum*1.3);
    if (chi2test&&hdata!=0) {
      stringstream sstest;
    ...
    if (setmu) {
      TTree* tree = (TTree*) hfile->Get("tree_fit_sb");
      Double_t mu_up=0;
      Double_t mu_down=0;
      tree->SetBranchAddress("muHiErr", &mu_up);
      tree->SetBranchAddress("muLoErr", &mu_down);

      tree->SetBranchAddress("mu", &mu);
      tree->GetEntry(0);
      //    TTree* htree = new hfile2->Get("tree_fit_sb");
      stringstream ssmu;

      ssmu<<"0.64:0.805:42:0.05:#mu = " <<fixed << showpoint << setprecision(1)<< mu << "^{ + " << mu_up << "}_{ -  "<< mu_down << "}";
      labels.push_back(ssmu.str());
      if(mu<=0) signalfac = 1;
    }
  ...
  cout << "here bla" << endl;
  cout << mcsignal.size() << endl;
  cout << "mcsignal[0]: " << mcsignal[0] << "   signalfac: " << signalfac << " " << endl;
  if(!mcsignal.empty()){
    TH1F* sig_i = mcsignal[0];
    sig_i->SetFillStyle(0);
    sig_i->SetLineColor(632);
    sig_i->SetLineWidth(2);
    if (setmu) sig_i->Scale(signalfac);
    else sig_i->Scale(signalfac);
    if(signalfac!=1){
      stringstream ss;
      ss<<signalfac;
      if(setmu){
	l->AddEntry(sig_i,(ss.str()+"x #mu x tHq").c_str(),"l");
      }
      else  l->AddEntry(sig_i,(ss.str()+"x tHq").c_str(),"l");
    }
    
      sig_i->Draw("HISTSAME");
      if(mcsignal.size()>1){
	TH1F* sig_i2 = mcsignal[1];
	sig_i2->SetFillStyle(0);
	sig_i2->SetLineColor(1308);
	sig_i2->SetLineWidth(2);
	sig_i2->Scale(signalfac);
	if(signalfac!=1){
	  stringstream ss;
	  ss<<signalfac;
	  if(setmu){
	    l->AddEntry(sig_i2,(ss.str()+"x #mu x tHW").c_str(),"l");
	  }
	  else  l->AddEntry(sig_i2,(ss.str()+"x tHW").c_str(),"l");
	  sig_i2->Draw("HISTSAME");
	}
      }
  }
  ...
  if (residuals) {
	...
	if(poisson)
	 ...
	 hsysstat->SetBinError(ibin,sqrt(herrges2->GetBinError(ibin)*herrges2->GetBinError(ibin)+staterr*staterr));
       ...
       herrges->SetFillColor(1306);
       herrges->SetFillStyle(3244);
       herrges->SetMarkerSize(0);
       herrges2->SetLineWidth(0);
       herrges2->SetFillColor(1306);
       herrges2->SetFillStyle(3244);
       herrges2->SetMarkerSize(0);

       hsysstat->SetLineWidth(0);
       hsysstat->SetFillColor(1306);
       hsysstat->SetFillStyle(3244);
       hsysstat->SetMarkerSize(0);

       herr2->SetLineColor(kBlack);
       herr2->SetFillColor(1306);
       herr2->SetFillStyle(3244);
       herr2->SetMarkerStyle(1);
       //hsysup->SetFillColor(kPink-7);
       //hsysup->SetFillStyle(3244);    
       //hsysup->SetLineWidth(0);
       //hsysdown->SetFillColor(kPink-7);
       //hsysdown->SetFillStyle(3244);
     ...
     hmc->GetYaxis()->SetNoExponent(true);
     hmc->GetXaxis()->SetNoExponent(true);
  ...
  {
    c->SaveAs(("prefit2D_MVA_"+channel+"_"+suffix+".pdf").c_str());
    c->SaveAs(("prefit2D_MVA_"+channel+"_"+suffix+".png").c_str());
  }
  else{
    c->SaveAs(("postfit2D_MVA_"+channel+""+suffix+".pdf").c_str());
    c->SaveAs(("postfit2D_MVA_"+channel+""+suffix+".png").c_str());
  }
...
TH1F* my_bin_change(TH1F* h, int nbinsx, double xmin, double xmax, int lowerbin, int higherbin)
{
  TH1F* new_h = new TH1F(h->GetName(), h->GetName(), nbinsx, xmin, xmax);
  for(int i=0; i < nbinsx; i++){
    new_h->SetBinContent(i+1, h->GetBinContent(lowerbin+i));
    new_h->SetBinError(i+1, h->GetBinError(lowerbin+i));
  }
  return new_h;
}
