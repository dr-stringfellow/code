...
int main(int argc,char**argv) {
  ...
  if (data.empty() && mcs.empty()) {
    cout << "provide -data and/or -mcs!" << endl;
    return 0;
  }
  if (batch && formats.size() == 0)
    formats.push_back("pdf");
  if (output.empty()) {
    output = (varname.empty()) ? "DataToMC" : varname;
    if (dirname != "utm")
      output = dirname + "_" + output;
    if (suffix.size() > 0)
      for (int i = (int) suffix.size() - 1; i >= 0; i--)
        output = output + "_" + suffix[i];
    if (prefix.size() > 0)
      for (int i = (int) prefix.size() - 1; i >= 0; i--)
        output = prefix[i] + "_" + output;
    if (!stackmc)
      output = output + "_ns";
    if (!opath.empty())
      output = opath + "/" + output;
  }
  if (mcs.size() > 0 && !reversemc)
    std::reverse(mcs.begin(), mcs.end());
  if ((data.empty() && mcref.empty()) || mcs.empty() || !stackmc)
    residuals = false;
  ...
  if (!datasample.empty())
    DataSample::printInfo(datasample);
  ...
  if (mcsamples.size() > 0)
    MCSample::printSampleInfos(mcsamples);
  if (mcsamples.size() > 5)
    leginplot = false;
  ...
  if (it != name2histo.end()) {
    ...
    if (varexpr.empty()) {
      cout << "varexpr must be set!" << endl;
      return 0;
    }
    if (nbinsx == 0) {
      cout << "nbinsx  must be set!" << endl;
      return 0;
    }
    if (xmin < -1E09) {
      cout << "xmin    must be set!" << endl;
      return 0;
    }
    if (xmax > +1E09) {
      cout << "xmax    must be set!" << endl;
      return 0;
    }
    if (varname.empty())
      histogram.setName("UtmDataToMC");
    else
  ...
  if (it_tot != name2histo.end()) {
    ...
    if (varname.empty())
      histogram_tot.setName("UtmDataToMC");
    else
      histogram_tot.setName(varname);
  ...
  int cheight = 800;
  TCanvas *theCanvas = new TCanvas(cname.c_str(), cname.c_str(), cwidth, cheight);
  ...
  for (unsigned icut = 0; icut < cuts.size(); icut++) {
    ...
    for (unsigned iign = 0; iign < ignore.size(); iign++)
      if (cut.find(ignore[iign]) != string::npos)
        ignore_cut = true;
    if (ignore_cut)
      continue;
    if (!tightsel.empty())
      tightsel += "&&";
    if (!loosenottightsel.empty())
      loosenottightsel += "&&";
    if (cut.find(tightcut) != string::npos) {
      if (!mm.empty())
        cout << "MM: tight cut = " << cut << endl;
      tightsel += cut;
  ...
  if (!histogram.selection().empty()) {
    loosesel = (loosesel.empty()) ? histogram.selection() : histogram.selection() + "&&" + loosesel;
    tightsel = (tightsel.empty()) ? histogram.selection() : histogram.selection() + "&&" + tightsel;
    loosenottightsel = (loosenottightsel.empty()) ? histogram.selection() : histogram.selection() + "&&" +
                                                                            loosenottightsel;
  }
  ...
  //
  if (!mcs.empty()) {
    legh = (leginplot) ? std::min(legh,0.33 / (mcsamples.size() + 2.)) : std::min(legh,0.90 / (mcsamples.size() + 2.));
  }
  ...
  if (!datasample.empty()) {
    ...
    if (histogram.variableBinWidth())
      Histogram::divideBinContentByBinWidth(hdata);
  }
  ...
  if (!datasample.empty() && !mm.empty()) {
    double eps_sig, err_eps_sig;
    get_val_and_err(eps_sig, err_eps_sig, epssig);
    double eps_qcd, err_eps_qcd;
    get_val_and_err(eps_qcd, err_eps_qcd, epsqcd);
    ...
    if (mm.find("E") != string::npos) {
      ...
      hmm = matrixmethod::getQcdTight(hDataLoose, hDataTight, eps_qcd, eps_sig, err_eps_qcd, err_eps_sig);
      hmm->SetTitle("");
      ...
      pos = epssig.find("+-");
      if (pos != string::npos)
        epssig = epssig.substr(0, pos);
      pos = epsqcd.find("+-");
      if (pos != string::npos)
        epsqcd = epsqcd.substr(0, pos);
      datasample.projectMM(hmm->GetName(), histogram.expr(), loosenottightsel, tightsel, epssig, epsqcd);
    }
  }
  if (!mm.empty() && 0 == hmm)
    cout << "mm='" << mm << "' invalid, MatrixMethod not applied!" << endl;
  if (0 != hmm && mm.find("DATA") != string::npos) {
    hdata = hmm;
    datalabel = "QCD (MM)";
  }
  if (0 != hdata) {
    ...
    l->AddEntry(hdata, datalabel.c_str(), "F");
  }
  ...
  if (mcs.size() > 0) {
    ...
    for (unsigned imc = 0; imc < mcsamples.size(); imc++) {
      ...
      stringstream sshmcname_i;
      sshmcname_i << "h" << mcsample.name();
      stringstream sshmcname_tot;
      sshmcname_tot << "htot" << mcsample.name();
      ...
      if (0 == hdata && mcsample.name() == mcref) {
        ...
        maximum = hdata->GetMaximum();
        l->AddEntry(hdata, get_leglabel(mclabels, mcsample.name()).c_str(), "F");
      } else {
  ...
  if (0!=hmm&&mm.find("MC")!=string::npos) {
    if (stackmc) {
      ...
      if (0==mchistos.size()) {
        ...
        if (chi2test&&hdata!=0) {
          stringstream sstest;
          sstest<< ((chi2test&&kstest) ? "0.62" : "0.77") << ":0.96:42:#chi^{2}=" << setprecision(4)\
                << get_chi2test(hdata,hmc);
          labels.push_back(sstest.str());
        }
        if (kstest&&hdata!=0) {
          stringstream sstest;
          sstest<<"0.77:0.96:42:KS=" << setprecision(3) << get_kstest(hdata,hmc);
          labels.push_back(sstest.str());
        }
      }
    } else {
      ...
      if (hmm->GetMaximum()>maximum)
        maximum = hmm->GetMaximum();
      if (get_minimum(hmm) <minimum)
        minimum = get_minimum(hmm);
      hmm->Draw("EP");
      l->AddEntry(hmm,"QCD (MM)","p");
      if (chi2test&&hdata!=0) {
        stringstream sstest;
        sstest<< ((chi2test&&kstest) ? "0.62" : "0.77") << ":0.96:42:#chi^{2}=" << setprecision(4)\
              << hdata->Chi2Test(hmm,"CHI2");
        labels.push_back(sstest.str());
      }
      if (kstest&&hdata!=0) {
	      stringstream sstest;
	      sstest << "0.77:0.96:42:KS=" << setprecision(3) << hdata->KolmogorovTest(hmm);
	      labels.push_back(sstest.str());
      }
    }
  } //end of: if (0!=hmm&&mm.find("MC")!=string::npos)
  ...
  if (mchistos.size()>0) {
    if (stackmc) {
      ...
      for (unsigned i=0;i<mchistos.size();i++) {
        ...
        if (h->GetEntries()>0 &&lumi==0.)
          h->Scale(1.0/h->Integral());
        else if(h->GetEntries()>0 &&lumi<0.)
          h->Scale(1.0/h_tot->Integral());
        cout << "Integral:" << h_tot->Integral() << endl;
        if (h->GetMaximum()>maximum)
          maximum=h->GetMaximum();
        if (get_minimum(h) <minimum)
          minimum=get_minimum(h);
      ...
      if (!histogram.logy())
        mchistos[0]->SetMinimum(0.0);
      else if (-1.0!=ymin)
        mchistos[0]->SetMinimum(ymin);
      else if (minimum>0.0)
        mchistos[0]->SetMinimum(minimum);

      TH1* h = (0!=hmm) ? hmm : mchistos[0];
      h->SetMaximum(maximum);
    } // end of: if (stackmc)
  } //end of: if (mchistos.size() > 0)
  ...
  if (residuals) {
    ...
    Double_t max = std::max(std::abs(hres->GetMinimum()), std::abs(hres->GetMaximum()));
    max *=1.05;
    max = std::min(49.9,max);
    hres->SetMinimum(-max);
    hres->SetMaximum(max);
    ...
    if (!fitresids.empty()) {
      ...
      for (Int_t iPar=0;iPar<fResiduals->GetNpar();iPar++)
	      cout << "iPar=" << iPar << ": " << fResiduals->GetParameter(iPar)\
  ...
  for (unsigned ifmt=0;ifmt<formats.size();ifmt++) {
    theCanvas->Print((output+"."+formats[ifmt]).c_str());
  }
...
void rm_CMS(vector<string>& labels)
{
  ...
  for (unsigned i=0;i<labels.size();i++) 
    if (labels[i].find("CMS")!=string::npos) {
      cms=i;
      break;
    }
...
double get_minimum(const TH1* h)
{
  ...
  for (int ibin=1;ibin<h->GetNbinsX();ibin++) {
    ...
    if (bincontent>0.0&&bincontent<result)
      result = bincontent;
  }
  return result;
}
...
bool get_val_and_err(double& val, double& err,const string& expr)
{
  ...
  pos = tmp.find('-');
  if (pos==string::npos) {
    cout << "get_val_and_err ERROR!" << endl;
    return false;
  }
...
void draw_stats(unsigned level,TH1* hdata,double xoffset)
{
  ...
  stringstream ssn;
  int ndata = (Histogram::hasVariableBinWidth(hdata)) ? (int)hdata->Integral("width") : (int)hdata->Integral();
  ...
  if (level>0)
    stats.DrawLatex(xoffset,0.89,ssn.str().c_str());
  if (level>1)
    stats.DrawLatex(xoffset,0.855,ssmean.str().c_str());
  if (level>2)
    stats.DrawLatex(xoffset,0.82,ssrms.str().c_str());
