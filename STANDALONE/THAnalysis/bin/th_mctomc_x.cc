 ...
 int main(int argc,char**argv)
 {
   ...
   vector<string> mcstackname = cl.getVector<string> ("mcstackname",         "");
   ...
   bool           normalized   = cl.getValue <bool>    ("normalized",       1);
   ...
   bool           masterbool = cl.getValue<bool>("masterbool",              "");
   ...
   if (output.empty()) {
	 ...
	 if ( !normalized) output = output + "_nn";
	 if (logy) output = output + "_log";
   ...
   for (unsigned i=0;i<ignore.size();i++)
     ...
     if (it!=name2histo.end()) {
       ...
       if (varname.empty())  histogram.setName("");
	 ...
	 if(normalized){
	   ...
	   for (unsigned i=0;i<mchistos.size();i++) {
	     ...
	     if(std::find(mcstackname.begin(), mcstackname.end(), samplename.c_str())!=mcstackname.end())
	     {
	       mcstack_vector.push_back(h);
	       mcstack=(TH1F*)h->Clone();
	     }
	     
	     //	 cout << mcstack_vector.size() << endl;
	     ...
	     h->GetYaxis()->SetTitle("Norm. to unit area");
	   ...
	   maximum=0.0;
	   float mcref_integral = 0;
	   float mcstack_integral = 0;
	   float ratio = 0;
	   float tth = 0;
	   float twhb = 0;
	   float wwh = 0;
     ...
     if (residuals) {
       ...
       for(int i=0;i<mcstack_vector.size();i++)
       {     
	 
	 TH1F* hres = (TH1F*)mcstack_vector[i]->Clone();
	 ...
	 for (int ibin=1;ibin<=hres->GetNbinsX();ibin++) {
	   Float_t nmc1  = mcstack_vector[i]->GetBinContent(ibin);
	   Float_t nmc2   = hreference->GetBinContent(ibin);
	   Float_t emc1  = mcstack_vector[i]->GetBinError(ibin);
	   
	   cout << "Referenz: " << nmc2 << " Approximation: " << nmc1 << endl;
	   if (nmc2>0) {
	     Float_t bin  = (nmc1)/nmc2;
	     Float_t err  = emc1/nmc1;
	 ...
	 //   if (i==0)    hres->Draw("EPHIST");
	 //else hres->Draw("EPHIST same");
	 
	 if (i==0)    hres->Draw("P");
	 else hres->Draw("P same");
