...
int main(int argc,char**argv)
{
  ...
  bool           drawratioonly = cl.getValue<bool>("drawratioonly",  false);
  ...
  if (mcs.size()>0) {
    ...
    for (unsigned imc=0;imc<mcsamples.size();imc++) {
      ...
      cout << "3a" << endl;
      //while(imc<mcsamples.size()-3)
      MCSample     mcsample_up = mcsamples_sysup[imc];
      cout << "3b" << endl;
      //while(imc<mcsamples.size()-3)
      MCSample     mcsample_down = mcsamples_sysdown[imc];
      cout << "4" << endl;
      ...
      mcsample_up.setWeights(mcweights);
      mcsample_down.setWeights(mcweights);
  ...
  //
  if (stats&&hdata&&!drawratioonly) draw_stats(stats,hdata);
  ...
  if (residuals) {
	...
	hsysup->SetBinContent(ibin,(nmc_up-nmc)/nmc_up);
        hsysup->SetBinError(ibin,0.0);
	hsysdown->SetBinContent(ibin,(nmc_down-nmc)/nmc_down);
  ...
  //TCanvas* c_ratio = new TCanvas(cname.c_str(),cname.c_str(),cwidth,cheight);
  TPad* redPad = (TPad*) mainPad->GetPad(2)->Clone();
  TCanvas *temp = new TCanvas("lala","lala",1200,400);
  TPad* redPad2 = (TPad*) redPad->DrawClone();
  redPad2->SetPad(0,0,1,1);
  temp->SaveAs((output+"ratio.png").c_str());
  

  //mainPad->cd(2)->Print((output+"ratio.png").c_str());
  ...
  //mainPad->cd(2)->SaveAs((output+"ratio.png").c_str());
  //
