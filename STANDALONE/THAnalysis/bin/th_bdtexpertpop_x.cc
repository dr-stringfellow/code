...
// ------------
//
// tool to classify a single top higgs event with  boosted decision trees (TMVA package)
//
// 
//
//    
...
int main(int argc,char**argv)
{
  ...
  */
  ...
  TString weightfile_th = TString(bdtpath) +"/"+ TString("THvsBkg_Global_BFGS_v3a.weights.xml");
  ...
  */
  ...
  for (unsigned ievt=0;ievt<nevt;ievt++) {
    ...
    if(hbbjtidx[0]==-99) {
      ...
      var2   =  TMath::Abs(p4jets[ljtidx].Eta());  //abs(thq_Eta_Recoil)
      var3   =  (jtcsvt[hbbjtidx[0]]>0.898)*(TMath::Abs(p4jets[hbbjtidx[0]].Eta())<2.4)+(jtcsvt[hbbjtidx[1]]>0.898)*(TMath::Abs(p4jets[hbbjtidx[1]].Eta())<2.4); //thq_NumBTag_Higgs
      var4   =  TMath::Log(hbbpt);    //log(thq_Pt_Higgs)
      var5   =  TMath::Log(jtpt[ljtidx]);  //log(thq_Pt_Recoil)
      var6   =  deltaR(p4jets[tophadjtidx[0]].Eta(),p4jets[tophadjtidx[0]].Phi(),p4jets[tophadjtidx[1]].Eta(),p4jets[tophadjtidx[1]].Phi()); //tt_DeltaR_Light
      var7   =  TMath::Log(tophadm);   //log(tt_Mass_TopHad)
      var8   =  (jtcsvt[tophadjtidx[0]]>0.898)*(TMath::Abs(p4jets[tophadjtidx[0]].Eta())<2.4)+(jtcsvt[tophadjtidx[1]]>0.898)*(TMath::Abs(p4jets[tophadjtidx[1]].Eta())<2.4); //tt_NumPassBTag_Light
      //      cout << var1 << " " << var2 << " " <<var3 << " " <<var4 << " " <<var5 << " " <<var6 << " " <<var7 << " " <<var8 << " " <<endl;
