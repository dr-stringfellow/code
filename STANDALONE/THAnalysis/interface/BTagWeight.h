...
// assume only two OP are used Tight in [1], and Medium in [0]
...
public:
 ...
 BTagSampleEfficiency(const char * filename) 
 {
     ...
     if(ff){
       ...
       cerr << "No BTagWeights, because file not found!";
     }
 }
 
 vector<BTagWeight::JetInfo> jetInfo(Short_t jtgenflv, float SF_CSVM, float SF_CSVT)
 {
   std::vector<BTagWeight::JetInfo> jInfo;
   MeanEfficiencies * m=0;
   if(abs(jtgenflv)==5) m= &bEff;
   else if(abs(jtgenflv)==4) m= &cEff;
   else m= &lEff;
   jInfo.push_back(BTagWeight::JetInfo(m->CSVM,SF_CSVM)); 
   jInfo.push_back(BTagWeight::JetInfo(m->CSVT,SF_CSVT)); 
   return jInfo;
 } 
