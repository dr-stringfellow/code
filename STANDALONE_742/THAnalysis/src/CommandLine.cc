...
bool CommandLine::handle_vars(string& line){
  string copy = line;
  vector<size_t> begin_var;
  vector<size_t> end_var;
  size_t pos = copy.find("[", 0);
  while(pos != string::npos)
    {
      begin_var.push_back(pos);
      size_t pos2 = copy.find("]", pos+1);
      if(pos2 == string::npos){
	cout << "Check configs! Missing ] somewhere!" << endl;
	return false;
      }
      else 
	end_var.push_back(pos2); 
      pos = copy.find("[",pos2+1);
    }
  
  int offset = 0;

  for(size_t i=0; i<begin_var.size(); i++){
    string envvar = copy.substr(begin_var.at(i)+1,end_var.at(i)-begin_var.at(i)-1);
    char* pPath; string envres="";
    pPath = getenv(envvar.c_str());
    if(pPath!=NULL)
      envres = getenv(envvar.c_str());
    else
      continue;
    line.replace(begin_var.at(i)+offset,end_var.at(i)-begin_var.at(i)+1, envres);
    offset += (envres.size() - envvar.size() - 2);
  }  
