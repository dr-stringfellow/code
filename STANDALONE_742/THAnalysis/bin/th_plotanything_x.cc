...
int main(int argc,char**argv)
{
  ...
  unsigned       nbinsy    = cl.getValue <unsigned>("nbinsy",        3000);
  int            nbinsx_iter  = cl.getValue <int>("nbinsx_iter",  -1);
  double         xmin      = cl.getValue <double>  ("xmin",            -3);
  double         xmax      = cl.getValue <double>  ("xmax",             3);
  double         ymin      = cl.getValue <double>  ("ymin",            -3);
  double         ymax      = cl.getValue <double>  ("ymax",             3);
  ...
  if (!mcsamples.empty()) {
    ...
    */
    ...
    TH2F* hptvseta = new TH2F("hptvseta","Pt vs. Eta",nbinsx,xmin,xmax,nbinsy,ymin,ymax);
