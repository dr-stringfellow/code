 ...
 int main(int argc,char**argv)
 {
   ...
   double         log_min   = cl.getValue <double>  ("log_min",           0.01);
   ...
   new TColor(1408,0.7686274509803922, 0.3058823529411765, 0.3215686274509804
              );
   new TColor(1407,0.39215686274509803, 0.7098039215686275, 0.803921568627451
              );
   new TColor(1406,0.360784313725490,0.622222222222222,0.766013071895425
	      );
   new TColor(1405,0.329411764705882,0.534640522875817,0.728104575163399
              );
   new TColor(1404,0.2980392156862745, 0.4470588235294118, 0.6901960784313725
              );
   new TColor(1403,0.3333333333333333, 0.6588235294117647, 0.40784313725490196
              );
   new TColor(1402,0.5058823529411764, 0.4470588235294118, 0.6980392156862745
              );
   new TColor(1401,0.8, 0.7254901960784313, 0.4549019607843137
              );
   ...
   maximum = (histogram.logy()) ? 100.*maximum : 1.65*maximum;
   ...
   if (mchistos.size()>0) {
	 ...
	 maximum = (histogram.logy()) ? 50.*hmc->GetMaximum():1.5*hmc->GetMaximum();
       ...
       hmc->SetMinimum(minimum);
       if (logy)
	 hmc->SetMinimum(log_min);
	 
       hmc->SetMaximum(maximum);
       hmc->GetHistogram()->SetMinimum(minimum);
       if(logy)
	 hmc->GetHistogram()->SetMinimum(log_min);
	 ...
	 for (unsigned i=1;i<mchistos.size();i++) {
	   for (int ibin=1;ibin<=mchistos[i]->GetNbinsX();ibin++) {
	     ...
	     cout << ibin << "th" <<  herr->GetBinContent(ibin) << endl;
 ...
 gdata->SetPoint(i,-99,-99);
  ...
  if (residuals) {
	...
	hsysstat->SetBinError(ibin,sqrt(herrges2->GetBinError(ibin)*herrges2->GetBinError(ibin)+staterr*staterr));
      ...
      herrges->SetFillColor(kYellow-7);
      herrges->SetFillStyle(1001);
      herrges->SetMarkerSize(0);
      herrges2->SetLineWidth(0);
      herrges2->SetFillColor(kYellow-7);
      herrges2->SetFillStyle(1001);
      herrges2->SetMarkerSize(0);
      
      hsysstat->SetLineWidth(0);
      hsysstat->SetFillColor(kYellow-7);
      hsysstat->SetFillStyle(1001);
      hsysstat->SetMarkerSize(0);
      

    
      herr2->SetFillColor(kYellow-7);
	...
	for (int ibin=1;ibin<=herr2->GetNbinsX();ibin++) {
	  ...
	  herr2->SetMarkerColor(0);
	  if (tmp_bincontent == 0){
		cout << "bin is empty";
		herr2->SetBinContent(ibin, 0.0);
		herr2->SetBinError(ibin, 0.0);

		}
	  cout << "Bin: " << ibin << " tmp bin content: " << tmp_bincontent << endl;
	  cout << "Bin: " << ibin << " tmp error: " << tmp_error << endl;
	  cout << "Bin: " << ibin << " Content: " << herr2->GetBinContent(ibin) << endl;
	  cout << "Bin: " << ibin <<" Error: " <<herr2->GetBinError(ibin) << endl;
	...
	//herrges2->Draw("error sameE2");
      ...
      line->SetLineStyle(3);
      line->Draw();
      TLine *line2 = new TLine(xmin,resmax*5/6,xmax,resmax*5/6);
      line2->SetLineColor(kBlack);
      line2->SetLineStyle(3);
      line2->Draw();
      TLine *line3 = new TLine(xmin,-resmax*5/6,xmax,-resmax*5/6);
