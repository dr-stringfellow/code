...
int main(int argc,char**argv)
{
  ...
  string         flavor    = cl.getValue<string> ("flavor",               "qcd");
  int            bflav     = cl.getValue<int>   ("bflav",                    0); // 0: shift all jets, 1: shift b-jets only, 2: shift light jets only
  int            onlysource = cl.getValue<int>  ("onlysource",               -1);
  ...
  if (bflav == 0){cout << "Shift all jets" << endl;}
  else if (bflav == 1){ cout << "Shift tonly b jets!"<<endl;}
  else if (bflav == 2){ cout << "Shift only light jets!"<<endl;}
  else            {cout<<"Error in variable bflav: "<<bflav<<endl;return 0;}

  // setup the standalone jet correction service

  // From https://twiki.cern.ch/twiki/bin/viewauth/CMS/JECUncertaintySources
  // Instantiate uncertainty sources
  //  const int nsrc = 17;
  // This composition is equal to SubTotalMC:
  const char* tmpsrcnames[17] =
  ...
  if (onlysource > -1){
    cout << "ONLY ";
    srcnames[0] = tmpsrcnames[onlysource];
  }
  

  cout << "using source " << srcnames[0] << endl;
  
  

  // Change flavor composition to one of the following
  // FlavorZJet","FlavorPhotonJet","FlavorPureGluon","FlavorPureQuark","FlavorPureCharm","FlavorPureBottom
  // if(flavor=="quark")
  // srcnames[4] = "FlavorPureQuark";
  
  
  std::vector<JetCorrectionUncertainty*> vsrc(nsrc);
  std::vector<JetCorrectionUncertainty*> vsrc_gluon(nsrc);
  std::vector<JetCorrectionUncertainty*> vsrc_quark(nsrc);
  std::vector<JetCorrectionUncertainty*> vsrc_bottom(nsrc);
  
  

  for (int isrc = 0; isrc < nsrc; isrc++) {
    const char *name = srcnames[isrc];
    const char *name_gluon = "FlavorPureGluon";
    const char *name_quark = "FlavorPureQuark";
    const char *name_bottom = "FlavorPureBottom";
    
    JetCorrectorParameters *p = new JetCorrectorParameters(jesfile, name);
    JetCorrectionUncertainty *unc = new JetCorrectionUncertainty(*p);    
    vsrc[isrc] = unc;

    if(onlysource == 4){
      p = new JetCorrectorParameters(jesfile, name_gluon);
      unc = new JetCorrectionUncertainty(*p);    
      vsrc_gluon[isrc] = unc;

      p = new JetCorrectorParameters(jesfile, name_quark);
      unc = new JetCorrectionUncertainty(*p);    
      vsrc_quark[isrc] = unc;
      
      p = new JetCorrectorParameters(jesfile, name_bottom);
      unc = new JetCorrectionUncertainty(*p);    
      vsrc_bottom[isrc] = unc;
    }
  ...
  for (unsigned iinput=0;iinput<inputs.size();iinput++) {
	...
	for (int ijt=0;ijt<njt15;ijt++) {
	  ...
	  for (int isrc = 0; isrc < nsrc; isrc++) {
	    ...
	    if(onlysource==4){
	      if(abs(i_jtgenflv[ijt]) == 21 || abs(i_jtgenflv[ijt]) == 9) { unc =  vsrc_gluon[isrc]; cout << "Gluon Jet" << endl; }
	      if(abs(i_jtgenflv[ijt]) <= 6  && abs(i_jtgenflv[ijt]) != 5) { unc =  vsrc_quark[isrc]; cout << "Quark Jet" << endl; }
	      if(abs(i_jtgenflv[ijt]) == 5 )                              { unc = vsrc_bottom[isrc]; cout << "Bootom Jet" << endl;}
	    }

	    unc->setJetPt(i_jtpt[ijt]);
	    unc->setJetEta(i_jteta[ijt]);
	  ...
	  if(bflav==2 && abs(i_jtgenflv[ijt]) == 5){
	    sum2_up = 0;
	    sum2_dw = 0;
	  }
	  
	  if(systematic == -1)
	    ptscale = 1. - sqrt(sum2_dw);
	  else 
	    ptscale = 1. + sqrt(sum2_up);
	  // sanity check
	  assert(ptscale != 0.);
