...
int main(int argc,char**argv)
{
  ...
  double         scalefact = cl.getValue <double>  ("scalefact",          1.5);
  ...
  bool           graphs    = cl.getValue <bool>    ("graphs",           false); 
  ...
  double legymin = (legtitle.empty()) ? legy : legy-0.02; if (residuals) legymin =1.-1.33*(1-legymin);
  ...
  if (mchistos.size()>0) {
	...
	l->AddEntry(herr,"MC stat.","F");
      ...
      for (unsigned i=0;i<mchistos.size();i++) {
	TH1F* h = mchistos[i];
	string drawopt = (0!=hmm) ? "HISTSAME" : (i==0) ? "HIST C" : "HISTSAME C";
	...
	//Drawing not histograms, but graphs

	Float_t y_vals[nbinsx];
	Float_t x_vals[nbinsx]; 
	
	for (int ibin=1;ibin<=h->GetNbinsX();ibin++) {
	  
	  y_vals[ibin-1]=h->GetBinContent(ibin);
	  x_vals[ibin-1]=h->GetXaxis()->GetBinCenter(ibin);
	}
	
	
	TGraph* gr = new TGraph(nbinsx,x_vals,y_vals);
	...
	gr->SetLineWidth(2);
	string drawopt_graph = (i==0) ? "AC" : "C";
 
	if(graphs)
	  {
	    if (gr->GetMaximum()>maximum) {
	      maximum=gr->GetMaximum();
	      
	    }
	    if(i==0)
	      {
		gr->GetYaxis()->SetRangeUser(0,scalefact*maximum);
		float borderdistance = (xmax-xmin)/(2*nbinsx);
		gr->GetXaxis()->SetTitle(xtitle.c_str());
		gr->GetYaxis()->SetTitle(ytitle.c_str());
		gr->GetXaxis()->SetRangeUser(xmin+borderdistance,xmax-borderdistance);
		
	      }
	    gr->Draw(drawopt_graph.c_str());
	  }
  ...
  if (residuals) {
    ...
    hres->GetYaxis()->SetLabelSize(0.16);
    hres->GetYaxis()->SetNdivisions(505);
    hres->GetXaxis()->SetTitleSize(0.19);
    hres->GetXaxis()->SetLabelSize(0.19);
	...
	TLegend* l2 = new TLegend(0.8275,0.73,0.9275,0.93);
	...
	TLegend* l2 = new TLegend(0.8225,0.75,0.999,0.93);
	l2->SetFillColor(10); l2->SetLineColor(10); l2->SetBorderSize(0); l2->SetTextSize(0.155);
	l2->AddEntry(herr2,"MC stat.","F");
	l2->Draw();
      }
    }
