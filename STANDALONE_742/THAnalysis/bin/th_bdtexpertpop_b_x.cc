...
int main(int argc,char**argv)
{
  ...
  reader_th->AddVariable( "log(glb_Pt_J1)",                                   &var2 );
  reader_th->AddVariable( "log(glb_MET)",                                     &var3 );
  ...
  reader_th->AddVariable( "abs(tHq_Eta_Recoil)",                              &var6 );
  //  reader_th->AddVariable( "glb_Sphericity" ,                                  &var9 );
  reader_th->AddVariable( "tt_MaxEta_Light",                                  &var7);
  reader_th->AddVariable( "log(tt_Mass_TopHad)",                              &var8);
  reader_th->AddVariable( "log(tt_DMass_TopHadWHad)" ,                        &var9);
  reader_th->AddVariable( "log(tt_MaxMass_BTopHadLight)",                     &var10);
  reader_th->AddVariable( "tHq_NumBTag_Higgs" ,                               &var11);
  reader_th->AddVariable( "tt_NumPassBTag_Light",                             &var12);
  reader_th->AddVariable( "tHq_DeltaR_BJetsHiggs",                            &var13);
  reader_th->AddVariable( "tt_DeltaR_Light",                                  &var14);
  reader_th->AddVariable( "tt_SumCharge_Light" ,                              &var15);
  reader_th->AddVariable( "tt_Charge_BTopHad",                                &var16);
  ...
  */
  ...
  TString weightfile_th = TString(bdtpath) +"/"+ TString("THvsBkg_Global_BFGS_v3b.weights.xml");
  ...
  if (0!=itree->FindBranch("mlpout_b")) itree->SetBranchStatus("mlpout_b",0);
  ...
  */
  // create mlpout_b branches in output tree
  Float_t  mlpout_b=-99; otree->Branch("mlpout_b",&mlpout_b, "mlpout_b/F");
  ...
  otree->Branch("popvar2",      &shat, "popvar2/F");
  otree->Branch("popvar3",      &var2, "popvar3/F");
  otree->Branch("popvar4",      &var3, "popvar4/F");
  otree->Branch("popvar5",      &var4, "popvar5/F");
  otree->Branch("popvar6",      &var5, "popvar6/F");
  otree->Branch("popvar7",      &coststh_rec, "popvar7/F");
  otree->Branch("popvar8",      &var6, "popvar8/F");
  otree->Branch("popvar9",      &sphericity, "popvar9/F");
  otree->Branch("popvar10",      &var7, "popvar10/F");
  otree->Branch("popvar11",      &var8, "popvar11/F");
  otree->Branch("popvar12",      &var9, "popvar12/F");
  otree->Branch("popvar13",      &var10, "popvar13/F");
  otree->Branch("popvar14",      &var11, "popvar14/F");
  otree->Branch("popvar15",      &var12, "popvar15/F");
  otree->Branch("popvar16",      &var13, "popvar16/F");
  otree->Branch("popvar17",      &var14, "popvar17/F");
  otree->Branch("popvar18",      &var15, "popvar18/F");
  otree->Branch("popvar19",      &var16, "popvar19/F");
  ...
  */
  ...
  for (unsigned ievt=0;ievt<nevt;ievt++) {
    ...
    for(int jets=0; jets<njt; jets++){
      TLorentzVector tmp;
      tmp.SetPtEtaPhiE(jtpt[jets],jteta[jets],jtphi[jets],jte[jets]);
    ...
    if(hbbjtidx[0]==-99) {
      ...
      shat   =  TMath::Log((p4alljets+p4lepw).M());  //log(glb_SqrtSHat)
      var2   =  TMath::Log(jtpt[0]);  //log(glb_Pt_J1)
      var3   =  TMath::Log(met); //log(glb_MET)  
      var4   =  TMath::Log(hbbpt);    //log(thq_Pt_Higgs)
      var5   =  TMath::Log(jtpt[ljtidx]);  //log(thq_Pt_Recoil)
      //var7   =  coststh_rec;  //thq_Cos_LepRecoil_TH
      
      var6   =  TMath::Abs(p4jets[ljtidx].Eta());  //abs(thq_Eta_Recoil)
      //var9   =  sphericity;   //glb_Sphericity
      var7  =  TMath::Max(TMath::Abs(p4jets[tophadjtidx[0]].Eta()),TMath::Abs(p4jets[tophadjtidx[1]].Eta())); //tt_MaxEta_Light
      var8  =  TMath::Log(tophadm);   //log(tt_Mass_TopHad)
      var9  =  TMath::Log(tophadm-tophadwm);  //log(tt_DMass_TopHadWHad)
      var10  =  TMath::Log(TMath::Max((p4jets[tophadjtidx[2]]+p4jets[tophadjtidx[0]]).M(),(p4jets[tophadjtidx[2]]+p4jets[tophadjtidx[1]]).M())); //log(tt_MaxMass_BTopHadLight)
      var11  =  (jtcsvt[hbbjtidx[0]]>0.898)*(TMath::Abs(p4jets[hbbjtidx[0]].Eta())<2.4)+(jtcsvt[hbbjtidx[1]]>0.898)*(TMath::Abs(p4jets[hbbjtidx[1]].Eta())<2.4); //thq_NumBTag_Higgs
      var12  =  (jtcsvt[tophadjtidx[0]]>0.898)*(TMath::Abs(p4jets[tophadjtidx[0]].Eta())<2.4)+(jtcsvt[tophadjtidx[1]]>0.898)*(TMath::Abs(p4jets[tophadjtidx[1]].Eta())<2.4); //tt_NumPassBTag_Light
      var13  =  hbbdr;  //thq_DeltaR_BJetsHiggs
      var14  =  deltaR(p4jets[tophadjtidx[0]].Eta(),p4jets[tophadjtidx[0]].Phi(),p4jets[tophadjtidx[1]].Eta(),p4jets[tophadjtidx[1]].Phi()); //tt_DeltaR_Light
      var15  =  (jtcharge[tophadjtidx[1]]+jtcharge[tophadjtidx[0]])*lepcharge; //tt_SumCharge_Light
      var16  =  jtcharge[tophadjtidx[2]]*lepcharge; //tt_Charge_BTopHad
      //var22  =  met;
    ...
    */
    //mlpout_b = reader_th->EvaluateMVA( "MLP method"  );

      mlpout_b = reader_th->EvaluateMVA( methodName  );
