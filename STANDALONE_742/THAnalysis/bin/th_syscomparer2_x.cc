...
int main(int argc,char**argv)
{
  ...
  if (!mcsamples.empty()) {
    ...
    tex.DrawLatex(0.200,0.95,"#sqrt{s}=8 TeV");
    tex.DrawLatex(0.4,0.95,"L=20 fb^{-1}");
    tex.DrawLatex(0.200,0.88,"Muon channel, 3T region");
    ...
    if(sys==1)
      tex.DrawLatex(0.58,0.95,"BTag heavy flavor");
    if(sys==2)
      tex.DrawLatex(0.58,0.95,"BTag light flavor");
    if(sys==3)
      tex.DrawLatex(0.62,0.95,"#bf{JER} systematics");
    if(sys==4)
      tex.DrawLatex(0.62,0.95,"#bf{JES} systematics");
    ...
    for (unsigned ifmt=0;ifmt<formats.size();ifmt++)
      {
	if(sys==1)
	  c1->Print((sprefix+"bdt_sysbtag_"+bdtout+"."+formats[ifmt]).c_str());
	else if(sys==2)
	  c1->Print((sprefix+"bdt_sysmistag_"+bdtout+"."+formats[ifmt]).c_str());
	else if(sys==3)
	  c1->Print((sprefix+"bdt_sysjer_"+bdtout+"."+formats[ifmt]).c_str());
	else if(sys==4)
	  c1->Print((sprefix+"bdt_sysjes_"+bdtout+"."+formats[ifmt]).c_str());
	else 
