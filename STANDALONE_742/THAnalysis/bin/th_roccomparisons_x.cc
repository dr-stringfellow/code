...
int main(int argc,char**argv)
{
  ...
  double         legymin   = cl.getValue <double>  ("legymin",       0.2);
  double         legymax   = cl.getValue <double>  ("legymax",       0.4);
  ...
  for(unsigned i=0; i<vec_hist.size(); i++)
    ...
    vec_hist[0]->GetYaxis()->SetLimits(0.0,1);
    ...
    vec_hist[0]->GetYaxis()->SetNdivisions(405);
    ...
    vec_hist[0]->Draw("Axis");
    vec_hist[0]->Draw("AC same");
    vec_hist[0]->GetXaxis()->SetTitle("Signal efficiency");
    vec_hist[0]->GetYaxis()->SetTitle("Background rejection");
    ...
    vec_hist[0]->GetXaxis()->SetTitleSize(0.04);
    vec_hist[0]->GetYaxis()->SetTitleSize(0.04);
    vec_hist[0]->GetXaxis()->SetLabelSize(0.04);
    vec_hist[0]->GetYaxis()->SetLabelSize(0.04);
    

    for(unsigned i=1;i<vec_hist.size();i++)
      {
	if (i==1) vec_hist[i]->SetLineColor(kAzure-7);
	vec_hist[i]->SetLineWidth(2);
	vec_hist[i]->Draw("AC same");
      }
    ...
    leg1->SetTextFont(42);
    leg1->AddEntry(vec_hist[0], "m (H) in reco training", "l");
    leg1->AddEntry(vec_hist[1], "m (H) not used in reco","l");
    // if(drawthree)
    ...
    //    string text_auc = string(h_int[0]);

    TLatex tex_auc;
    tex_auc.SetTextSize(0.04);
    tex_auc.SetTextFont(42);
    tex_auc.SetNDC(true);
    tex_auc.SetTextColor(8);

    stringstream auc_0 ; auc_0 << h_int[0];
    string auc_0string = auc_0.str();

    tex_auc.DrawLatex(0.65,0.85,"AUC: ");
    tex_auc.DrawLatex(0.73,0.85,auc_0string.c_str());

    TLatex tex_auc_1;
    tex_auc_1.SetTextSize(0.04);
    tex_auc_1.SetTextFont(42);
    tex_auc_1.SetNDC(true);
    tex_auc_1.SetTextColor(kAzure-7);

    stringstream auc_1 ; auc_1 << h_int[1];
    string auc_1string = auc_1.str();

    tex_auc_1.DrawLatex(0.65,0.8,"AUC: ");
    tex_auc_1.DrawLatex(0.73,0.8,auc_1string.c_str());
