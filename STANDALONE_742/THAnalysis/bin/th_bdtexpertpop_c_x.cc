...
int main(int argc,char**argv)
{
  ...
  reader_th->AddVariable( "abs(tHq_Eta_Recoil)",                              &var8 );
  reader_th->AddVariable( "glb_Sphericity" ,                                  &var9 );
  reader_th->AddVariable( "tt_MaxEta_Light",                                  &var10);
  reader_th->AddVariable( "log(tt_Mass_TopHad)",                              &var11);
  reader_th->AddVariable( "log(tt_DMass_TopHadWHad)" ,                        &var12);
  reader_th->AddVariable( "log(tt_MaxMass_BTopHadLight)",                     &var13);
  reader_th->AddVariable( "tHq_NumBTag_Higgs" ,                               &var14);
  reader_th->AddVariable( "tt_NumPassBTag_Light",                             &var15);
  reader_th->AddVariable( "tHq_DeltaR_BJetsHiggs",                            &var16);
  reader_th->AddVariable( "tt_DeltaR_Light",                                  &var17);
  reader_th->AddVariable( "tt_SumCharge_Light" ,                              &var18);
  reader_th->AddVariable( "tt_Charge_BTopHad",                                &var19);
  ...
  */
  ...
  TString weightfile_th = TString(bdtpath) +"/"+ TString("THvsBkg_Global_BFGS_v3c.weights.xml");
  ...
  if (0!=itree->FindBranch("mlpout_c")) itree->SetBranchStatus("mlpout_c",0);
  ...
  Float_t  mlpout_c=-99; otree->Branch("mlpout_c",&mlpout_c, "mlpout_c/F");
  ...
  otree->Branch("popvar9",      &var9, "popvar9/F");
  otree->Branch("popvar10",      &var10, "popvar10/F");
  otree->Branch("popvar11",      &var11, "popvar11/F");
  otree->Branch("popvar12",      &var12, "popvar12/F");
  otree->Branch("popvar13",      &var13, "popvar13/F");
  otree->Branch("popvar14",      &var14, "popvar14/F");
  otree->Branch("popvar15",      &var15, "popvar15/F");
  otree->Branch("popvar16",      &var16, "popvar16/F");
  otree->Branch("popvar17",      &var17, "popvar17/F");
  otree->Branch("popvar18",      &var18, "popvar18/F");
  otree->Branch("popvar19",      &var19, "popvar19/F");
  ...
  */
  ...
  for (unsigned ievt=0;ievt<nevt;ievt++) {
    ...
    if(hbbjtidx[0]==-99) {
      ...
      var5   =  TMath::Log(hbbpt);    //log(thq_Pt_Higgs)
      var6   =  TMath::Log(jtpt[ljtidx]);  //log(thq_Pt_Recoil)
      var7   =  coststh_rec;  //thq_Cos_LepRecoil_TH
      
      var8   =  TMath::Abs(p4jets[ljtidx].Eta());  //abs(thq_Eta_Recoil)
      var9   =  sphericity;   //glb_Sphericity
      var10  =  TMath::Max(TMath::Abs(p4jets[tophadjtidx[0]].Eta()),TMath::Abs(p4jets[tophadjtidx[1]].Eta())); //tt_MaxEta_Light
      var11  =  TMath::Log(tophadm);   //log(tt_Mass_TopHad)
      var12  =  TMath::Log(tophadm-tophadwm);  //log(tt_DMass_TopHadWHad)
      var13  =  TMath::Log(TMath::Max((p4jets[tophadjtidx[2]]+p4jets[tophadjtidx[0]]).M(),(p4jets[tophadjtidx[2]]+p4jets[tophadjtidx[1]]).M())); //log(tt_MaxMass_BTopHadLight)
      var14  =  (jtcsvt[hbbjtidx[0]]>0.898)*(TMath::Abs(p4jets[hbbjtidx[0]].Eta())<2.4)+(jtcsvt[hbbjtidx[1]]>0.898)*(TMath::Abs(p4jets[hbbjtidx[1]].Eta())<2.4); //thq_NumBTag_Higgs
      var15  =  (jtcsvt[tophadjtidx[0]]>0.898)*(TMath::Abs(p4jets[tophadjtidx[0]].Eta())<2.4)+(jtcsvt[tophadjtidx[1]]>0.898)*(TMath::Abs(p4jets[tophadjtidx[1]].Eta())<2.4); //tt_NumPassBTag_Light
      var16  =  hbbdr;  //thq_DeltaR_BJetsHiggs
      var17  =  deltaR(p4jets[tophadjtidx[0]].Eta(),p4jets[tophadjtidx[0]].Phi(),p4jets[tophadjtidx[1]].Eta(),p4jets[tophadjtidx[1]].Phi()); //tt_DeltaR_Light
      var18  =  (jtcharge[tophadjtidx[1]]+jtcharge[tophadjtidx[0]])*lepcharge; //tt_SumCharge_Light
      var19  =  jtcharge[tophadjtidx[2]]*lepcharge; //tt_Charge_BTopHad
      //var22  =  met;
    ...
    */
    //mlpout = reader_th->EvaluateMVA( "MLP method"  );
      ...
      mlpout_c = reader_th->EvaluateMVA( methodName  );
