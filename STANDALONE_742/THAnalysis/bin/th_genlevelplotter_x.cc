...
int main(int argc,char**argv)
{
  ...
  string         reslabel  = cl.getValue <string>  ("reslabel","#frac{(syst.-nom.)}{nom.}");
  ...
  bool           plotps    = cl.getValue <bool>    ("plotps",              0);
  int            whichtodraw = cl.getValue <int> ("whichtodraw",         2);
  ...
  //
  // Introduced to show a second dashed band in the residuals. Parton-shower-only effect!
  //
  vector<MCSample> mcsamples_4;
  mcsamples_4=MCSample::loadSamples(mcs_2,dirname,treename,mcpath+mcsubpath,filesuffix);
  if (mcsamples_4.size()>0) MCSample::printSampleInfos(mcsamples_2);

  vector<MCSample> mcsamples_5;
  mcsamples_5=MCSample::loadSamples(mcs_3,dirname,treename,mcpath+mcsubpath,filesuffix);
  if (mcsamples_5.size()>0) MCSample::printSampleInfos(mcsamples_3);
  ...
  vector<TH1F*>  mchistos_4;
  vector<TH1F*>  mchistos_5;
  ...
  if (mcs.size()>0) {
    ...
    for (unsigned imc=0;imc<mcsamples.size();imc++) {
      ...
      MCSample     mcsample_4 = mcsamples_4[imc]; // separate histograms 
      MCSample     mcsample_5 = mcsamples_5[imc]; // separate histograms          
	...
	mcsample_3.setWeights(mcweights_3);
      ...
      //
      // For parton-shower-only: use nominal mcweights
      //

      mcsample_4.setWeights(mcweights);
      mcsample_5.setWeights(mcweights);
      ...
      TH1F* hmc_i_4 = histogram.create(sshmcname_i.str()+"_4");
      TH1F* hmc_i_5 = histogram.create(sshmcname_i.str()+"_5");
      ...
      if(usesyssamples)
	{
	  ...
	  mcsample_4.project(hmc_i_4->GetName(),histogram.expr(),mcsel);
          mcsample_5.project(hmc_i_5->GetName(),histogram.expr(),mcsel);
	...
	mchistos_4.push_back(hmc_i_4);
        mchistos_5.push_back(hmc_i_5);
  ...
  if (lumi>0.0){
    ...
    for (unsigned i=0;i<mchistos_4.size();i++)
      {
        mchistos_4[i]->Scale(lumi);
	if(drawnormalized)
	  mchistos_4[i]->Scale(1./mchistos_4[i]->Integral());
      }
    for (unsigned i=0;i<mchistos_5.size();i++)
      {
        mchistos_5[i]->Scale(lumi);
        if(drawnormalized)
          mchistos_5[i]->Scale(1./mchistos_5[i]->Integral());
      }
    ...
    for (unsigned i=0;i<mchistos.size();i++) {
      ...
      h_3->Scale(Idata/Imc);
      TH1F* h_4 = mchistos_4[i];
      h_4->Scale(Idata/Imc);
      TH1F* h_5 = mchistos_5[i];
      h_5->Scale(Idata/Imc);
  ...
  if (mchistos.size()>0) {
	...
	MCInt+= mchistos[i]->Integral();
      ...
      for (unsigned i=0;i<mchistos_2.size();i++) {
	hmc_2->Add(mchistos_2[i]);
	MCInt_2+= mchistos_2[i]->Integral();
      }
      if (0==hmc_3) hmc_3 = new THStack("hmc_3","hmc_3");
      for (unsigned i=0;i<mchistos_3.size();i++) {
	hmc_3->Add(mchistos_3[i]);
	MCInt_3+= mchistos_3[i]->Integral();
      }
      if (0==hmc_4) hmc_4 = new THStack("hmc_4","hmc_4");
      for (unsigned i=0;i<mchistos_4.size();i++) hmc_4->Add(mchistos_4[i]);
      if (0==hmc_5) hmc_5 = new THStack("hmc_5","hmc_5");
      for (unsigned i=0;i<mchistos_5.size();i++) hmc_5->Add(mchistos_5[i]);
      ...
      cout << "Nominal set: " << MCInt << " Set 2: " << MCInt_2 << " Set 3: " << MCInt_3 << endl;
  ...
  if (residuals) {
    ...
    for (int ibin=1;ibin<=hres_2->GetNbinsX();ibin++) {
      ...
      Float_t nmc_4  = ((TH1*)hmc_4->GetStack()->Last())->GetBinContent(ibin);
      Float_t nmc_5  = ((TH1*)hmc_5->GetStack()->Last())->GetBinContent(ibin);
	...
	hres_4->SetBinContent(ibin,(nmc_4-nmc)/nmc);
        hres_4->SetBinError(ibin,0.0);
	hres_5->SetBinContent(ibin,(nmc_5-nmc)/nmc);
        hres_5->SetBinError(ibin,0.0);
	...
	if(return_sign(hres_4->GetBinContent(ibin))==return_sign(hres_5->GetBinContent(ibin)))
          {
            if(abs(hres_4->GetBinContent(ibin))>abs(hres_5->GetBinContent(ibin)))
              {
                herrps->SetBinContent(ibin,hres_4->GetBinContent(ibin)/2);
                herrps->SetBinError(ibin,hres_4->GetBinContent(ibin)/2);
              }
            else
              {
                herrps->SetBinContent(ibin,hres_5->GetBinContent(ibin)/2);
                herrps->SetBinError(ibin,hres_5->GetBinContent(ibin)/2);
              }
          ...
          {
            herrps->SetBinContent(ibin,(hres_4->GetBinContent(ibin)-hres_5->GetBinContent(ibin))/2+hres_5->GetBinContent(ibin));
            herrps->SetBinError(ibin,abs((hres_4->GetBinContent(ibin)-hres_5->GetBinContent(ibin)))/2);
          }

      }
      else if(nmc==0&&nmc_3!=0 && nmc_2==0){
	...
	hsysstat->SetBinError(ibin,sqrt(herrges2->GetBinError(ibin)*herrges2->GetBinError(ibin)));
      ...
      herrges2->SetFillStyle(3005);
      herrges2->SetMarkerSize(0);
      
      herrps->SetLineWidth(1);
      herrps->SetFillColor(kGray+3);
      herrps->SetFillStyle(3002);
      herrps->SetMarkerSize(0);
	...
	if(plotsys) 
	  {
	    if (whichtodraw == 0 || whichtodraw == 2)
	      hsysstat->Draw("error sameE2");
	    if(plotps && (whichtodraw == 2 || whichtodraw == 1))
	      herrps->Draw("error sameE2");
	  }
	...
	TLegend* l2 = new TLegend(0.825,0.68,0.9575,0.93);
	...
	if(plotps && whichtodraw>=1)
	  l2->AddEntry(herrps,"PS only","F");
	if(plotsys)
	  l2->Draw();
      }
      else{
