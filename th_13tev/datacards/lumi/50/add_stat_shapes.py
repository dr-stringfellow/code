        ...
        for histo in histos:
            ...
            if do_shapes:
                ...
                for ibin in range(1, th1.GetNbinsX()+1):
                    ...
                    if above_threshold:
                        myString = th1.GetName().replace("__nominal",'')
                        process = myString[myString.rfind("__")+2:]
                        process = process.replace("_",'')
                        channel = myString[:myString.find("__")]
                        # shorten channel name
                        channel = channel.replace("tH_",'').replace("tt_",'')
                        writtensys=("CMS_tHq_%s%s_bin%i" % (process, channel, ibin))
                        err_up = th1.Clone(
                            th1.GetName().replace("__nominal",'') + "__%sUp" % (writtensys))
                        err_down = th1.Clone(
                            th1.GetName().replace("__nominal",'') + "__%sDown" % (writtensys))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--prefix', default='tH',
