...
while True:
    ...
    if prefit and norm_p and errors:
        print " %-8s & %7.1f \pm %7.1f & %7.1f \pm %7.1f " % ( m.group(2),  norm_p.getVal(), norm_p.getError(), norm_s.getVal(), norm_s.getError())
    else:
        if errors:
            print "%-30s %-30s %7.1f \pm %7.1f  %7.1f \pm %7.1f" % (m.group(1), m.group(2), norm_s.getVal(), norm_s.getError(), norm_b.getVal(), norm_b.getError())
        else:
