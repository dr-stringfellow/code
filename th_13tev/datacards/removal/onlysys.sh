...
for i in $(cat syst_wobbb.txt); 
  ...
  combine -M Asymptotic -d higgsCombine_0.MultiDimFit.mH120.root -w w -n _only_0 --snapshotName MultiDimFit --toysFrequentist --bypassFreq  --freezeNuisances ${freezing%,}| grep "Expected\ 50.0\%" |tee -a  onlysys_effect.txt; 
done
