...
public:
  ...
  CMSStyle(){
    ...
    cmsStyle.SetPaintTextFormat("1.4f");
...
// For the statistics box:
  ...
  cmsStyle.SetStatFontSize(0.05);
  cmsStyle.SetStatTextColor(1);
  cmsStyle.SetStatFormat("5.4g");
...
// For the axis titles:
  ...
  //cmsStyle.SetTitleOffset(1.1, "XY"); // Another way to set the Offset
...
// For the axis:
  ...
  cmsStyle.SetNdivisions(505, "YZ");
  cmsStyle.SetNdivisions(310, "X");
...
void LPlusJets(){
    TLatex* text1 = new TLatex(3.570061,23.08044,"#ell + jets");
    text1->SetNDC();
    text1->SetTextAlign(13);
    text1->SetX(0.194);
    text1->SetY(0.928-4*0.048);
    text1->SetTextFont(42);
    text1->SetTextSizePixels(24);
    text1->Draw("same");}
...
void CMSSimulation_2D(){
  ...
  void With2DEdges(TH2* histo){
    int xbin=histo->GetNbinsX();
    int ybin=histo->GetNbinsY();
    TLine* linex[xbin];
    TLine* liney[ybin];
    cout << "TEST: " << xbin << " " << ybin << endl;
    for(int x=1; x<xbin; x++){
      linex[x-1]= new TLine(histo->GetXaxis()->GetBinUpEdge(x),histo->GetYaxis()->GetBinLowEdge(histo->GetYaxis()->GetFirst()),histo->GetXaxis()->GetBinUpEdge(x),histo->GetYaxis()->GetBinUpEdge(histo->GetYaxis()->GetLast()));
      	 
    }
    
 for(int y=1; y<ybin; y++){
      liney[y-1]= new TLine(histo->GetXaxis()->GetBinLowEdge(histo->GetXaxis()->GetFirst()),histo->GetYaxis()->GetBinUpEdge(y),histo->GetXaxis()->GetBinUpEdge(histo->GetXaxis()->GetLast()),histo->GetYaxis()->GetBinUpEdge(y));
      	 
    }

 for(int x=0; x<xbin-1; x++){
   linex[x]->SetLineWidth(1);
   linex[x]->Draw("same");
 }
for(int y=0; y<ybin-1; y++){
  liney[y]->SetLineWidth(1);
  liney[y]->Draw("same");
 }
  }

  void TextforCMS(string text,float row){ 
...
void fixOverlay() {
 ...
 void setup_style_2D(TH2* histo, std::string xtitle, std::string ytitle)
  ...
  void setup_style_2D(TH2* histo, std::string xtitle, std::string ytitle, std::string ztitle)
  {histo->UseCurrentStyle();
    cmsStyle.SetPadRightMargin(0.03);
    histo->SetTitle("");      
    histo->SetXTitle(xtitle.c_str());
  ...
  TPaveText* ShowResults(TH1* graph){

  TPaveText* pav=new TPaveText(0.6,0.75,0.94,0.94,"brNDC");
  
  double mean = graph->GetFunction("gaus")->GetParameter(1);
  double mean_err = graph->GetFunction("gaus")->GetParError(1);
  double sigma = graph->GetFunction("gaus")->GetParameter(2);
  double sigma_err = graph->GetFunction("gaus")->GetParError(2);

  pav->SetTextAlign(13);
  pav->SetFillColor(0);
  pav->SetTextFont(42);
  pav->SetTextSizePixels(24);
  char name[30];
  sprintf(name, "gaussian fit:");
  pav->AddText(name);
  sprintf(name, "mean = %1.3f +- %1.3f", mean, mean_err);
  pav->AddText(name);
  sprintf(name, "sigma = %1.3f +- %1.3f", sigma, sigma_err);
  pav->AddText(name);
  pav->Draw("same");
