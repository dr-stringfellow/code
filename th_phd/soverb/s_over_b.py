...
def plot(inputfit, output, inputdata, bins, mcunc_on_bck):
    ...
    c = TCanvas("canvas","canvas", 800, 2000)
    ...
    c.Divide(1,4,0.0,0.0)
    
    c.GetPad(1).SetFillColor(0)
    c.GetPad(2).SetFillColor(0)
    c.GetPad(3).SetFillColor(0)
    c.GetPad(4).SetFillColor(0)
    
    c.GetPad(1).SetPad(0.0,0.40,1.0,1.0)
    c.GetPad(2).SetPad(0.0,0.30,1.0,0.40)
    c.GetPad(3).SetPad(0.0,0.20,1.0,0.30)
    c.GetPad(4).SetPad(0.0,0.0,1.0,0.20)
    ...
    #label_kstest.DrawLatex(0.46,0.8,"KS test = %3.2f    #chi^{2} = %3.2f" % (kstest,chi2))


    # Stat.+syst. label

    l_res = TLegend(0.18,0.68,0.4,0.93)
    ...
    label_kstest_sb.SetTextSize(0.18)
    label_kstest_sb.SetTextFont(42)
    #label_kstest_sb.DrawLatex(0.46,0.8,"KS test = %3.2f    #chi^{2} = %3.2f" % (kstest_sb,chi2_sb))


    # Stat.+syst. label

    l_res_sb = TLegend(0.18,0.68,0.40,0.93)
    l_res_sb.SetTextSize(0.18)
    ...
    hres_purity = hres.Clone("hres_purity")
    hres_purity.Reset()
    hres_purity.GetYaxis().CenterTitle()
    hres_purity.SetStats(0)

    
    hres_purity.GetYaxis().SetTitleOffset(2.6)
    hres_purity.GetYaxis().SetTitleSize(20)
    hres_purity.GetYaxis().SetLabelSize(24)
    hres_purity.GetYaxis().SetLabelOffset(0.004)
    hres_purity.GetYaxis().SetNdivisions(505)
    hres_purity.GetXaxis().SetTitleSize(40)
    hres_purity.GetXaxis().SetTickLength(hres_purity.GetXaxis().GetTickLength() * 0.6 )
    hres_purity.SetYTitle("#frac{#it{S}}{#it{B}}")
    hres_purity.SetXTitle("log_{10}(#it{S}/#it{B})")
    hres_purity.GetXaxis().SetTitleOffset(344)
    hres_purity.SetMinimum(0.0001)
    hres_purity.SetMaximum(9)
    hres_purity.SetMarkerStyle(20)
    hres_purity.SetMarkerSize(0.5)
    hres_purity.GetXaxis().SetTitleOffset(5)



    hres_purity.SetMarkerSize(0)
    hres_purity.SetLineWidth(2)
    #gPad.SetGridy()
    hres_purity.Draw("AXIS")
    hres_purity.Draw("AXIG same")
               


    hres_soverb_unc = hres.Clone("hres_soverb")
    hres_soverb_unc.Reset()


    for ibin in range(1,mchistos[i].GetNbinsX()+1) :
        hres_purity.SetBinContent(ibin,h_s.GetBinContent(ibin)/h_b.GetBinContent(ibin))
        #helper_max = (h_s.GetBinContent(ibin)+h_s.GetBinError(ibin))/(h_b.GetBinContent(ibin)-h_b.GetBinError(ibin))
        #if h_b.GetBinContent(ibin)-h_b.GetBinError(ibin) <0:
        #    helper_max = 999 
        #helper_min = (h_s.GetBinContent(ibin)-h_s.GetBinError(ibin))/(h_b.GetBinContent(ibin)+h_b.GetBinError(ibin))
        #if h_s.GetBinContent(ibin)-h_s.GetBinError(ibin) <0:
        #    helper_min = 0.00001
        #hres_soverb_unc.SetBinContent(ibin,(helper_max+helper_min)/2)
        #hres_soverb_unc.SetBinError(ibin,(helper_max-helper_min)/2)
        #if hres_soverb_unc.GetBinContent(ibin)>5:
        #    hres_soverb_unc.SetBinContent(ibin,hres_purity.GetBinContent(ibin))
        #    hres_soverb_unc.SetBinError(ibin,5)
        hres_soverb_unc.SetBinContent(ibin,h_s.GetBinContent(ibin)/h_b.GetBinContent(ibin))
        helper = math.sqrt(pow(h_s.GetBinError(ibin),2)*1/h_b.GetBinContent(ibin)+pow(h_b.GetBinError(ibin),2)*pow((h_s.GetBinContent(ibin)/pow(h_b.GetBinContent(ibin),2)),2))
        hres_soverb_unc.SetBinError(ibin,helper)
        print "UNCERTAINTY" + str(helper)
        print "CONTENT" + str(hres_soverb_unc.GetBinContent(ibin))

    hres_purity.SetFillColor(0)
    hres_purity.SetLineColor(797)
    hres_purity.Draw("error sameE2")

    hres_soverb_unc.SetFillColor(796)
    hres_soverb_unc.SetLineColor(0)
    hres_soverb_unc.SetFillStyle(3001)

    hres_soverb_unc.SetMinimum(0.0001)
    hres_soverb_unc.SetMaximum(8)
    hres_soverb_unc.Draw("error sameE2")


    # Stat.+syst. label

    l_res_soverb = TLegend(0.175,0.81,0.42,0.96)
    l_res_soverb.SetTextSize(0.09)
    l_res_soverb.SetFillColor(0)
    l_res_soverb.SetFillStyle(0)
    l_res_soverb.SetLineColor(10) 
    l_res_soverb.SetBorderSize(0)

    l_res_soverb.AddEntry(hres_soverb_unc,"#bf{Sig.+bkg. stat.+sys.}","F")
    l_res_soverb.Draw()
    ...
    stats.DrawLatex(0.18,0.86,"Muon + electron channel")
    stats.DrawLatex(0.18,0.79,"3 + 4 tag region")
    ...
    l.AddEntry(h_d,"Data","ep")
    #l.AddEntry(h_s,"tHq, y#scale[0.55]{t}=-1","f")
    l.AddEntry(h_s,"tHq #scale[0.8]{(^{}#it{C}_{t}=-1)}","f")
