...
outDirectory = 'fig_PAS-style_alt-legend/'
...
roc.SetTitle(';Correct interpretation selection eff.;Wrong interpretation rejection eff.')
roc.SetMinimum(0.)
roc.SetMaximum(1.)
roc.SetLineWidth(2)
roc.Draw('c')
...
hStack = ROOT.THStack('hStack', ';NN response;Probability density [a. u.]')
...
# Create a legend
#legend = ROOT.TLegend(0.18, 0.63, 0.45, 0.85)
legend = ROOT.TLegend(0.18, 0.70, 0.65, 0.85)
legend.SetFillColor(ROOT.kWhite)
legend.SetTextSize(0.04)
legend.SetNColumns(2)

legend.SetHeader('Event interpretation')
legend.AddEntry(hSgnExam, 'correct exam', 'f');
legend.AddEntry(hBkgExam, 'wrong exam', 'f');
legend.AddEntry(hSgnTrain, 'correct train', 'p');
legend.AddEntry(hBkgTrain, 'wrong train', 'p');
...
hStack.Draw('nostack')
hStack.SetMinimum(0.)
hStack.SetMaximum(0.20)  # Fine tunning
