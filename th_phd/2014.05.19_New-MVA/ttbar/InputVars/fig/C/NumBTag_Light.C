...
//=========  (Mon May 19 14:43:51 2014) by ROOT version5.34/09
...
// ------------>Primitives in pad: mainPad
   ...
   mainPad->Draw();
   mainPad->cd();
   mainPad->Range(-0.875,-0.09260239,2.875,0.8334214);
   ...
   hStack->SetTitle("#tagged(q_{thad});#tagged(q_{thad});Probability density");
   Double_t xAxis97[4] = {-0.5, 0.5, 1.5, 2.5}; 
   
   TH1F *hStack_stack_33 = new TH1F("hStack_stack_33","#tagged(q_{thad})",3, xAxis97);
   ...
   TPaveText *pt = new TPaveText(0.3715496,0.9157254,0.6284504,0.995,"blNDC");
   pt->SetName("title");
   pt->SetBorderSize(0);
   pt->SetFillColor(0);
   pt->SetFillStyle(0);
   pt->SetTextFont(42);
   TText *text = pt->AddText("#tagged(q_{thad})");
   pt->Draw();
   mainPad->Modified();
   canvas->cd();
